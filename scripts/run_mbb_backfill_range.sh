#!/usr/bin/env bash
# Multi-season NCAA MBB backfill campaign: seasons NEWEST-first, chunked capture
# per season until complete, resumable at every level. Wraps run_mbb_backfill.sh
...
#   ./scripts/run_mbb_backfill_range.sh                 # 2025 down to 2010
#   ./scripts/run_mbb_backfill_range.sh 2025 2020       # 2025 down to 2020
#   CHUNK=1000 COOLDOWN_S=900 ./scripts/run_mbb_backfill_range.sh
#
...
season = '$1'
m = pl.read_parquet('mbb/schedule_master.parquet')
total = m.filter(pl.col('season') == season).height
have = len(list(pathlib.Path('mbb/raw', season).glob('*'))) if pathlib.Path('mbb/raw', season).is_dir() else 0
...
for season in $(seq "$START" -1 "$END"); do
  ...
  while :; do
    ...
    NCAA_VENDOR="$VENDOR" CHUNK="$CHUNK" WORKERS="$WORKERS" \
      ./scripts/run_mbb_backfill.sh "$season" 2>&1 | tee -a "$LOG"
    rc=${PIPESTATUS[0]}
