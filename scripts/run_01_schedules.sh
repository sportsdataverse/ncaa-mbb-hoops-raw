...
#   mbb/schedules/html/{season}/{team_id}.html   -- the raw page
#   mbb/schedules/json/{season}/{team_id}.json   -- team/opponent ids AND names
# The compiled mbb/schedules/parquet/{season}.parquet is built separately by
...
rc=0
if [[ " $* " == *" --shard "* ]]; then
  "$PY" python/ncaa_mbb_01_schedules_scrape.py "$@" 2>&1 | tee -a "${LOG}"
  rc=${PIPESTATUS[0]}
else
  pids=()
  for i in $(seq 0 $((DISCOVER_WORKERS - 1))); do
    "$PY" python/ncaa_mbb_01_schedules_scrape.py "$@" --shard "${i}/${DISCOVER_WORKERS}" >> "${LOG}" 2>&1 &
    pids+=($!)
  done
