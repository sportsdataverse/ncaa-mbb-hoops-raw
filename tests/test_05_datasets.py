...
import ncaa_mbb_05_datasets_build as nd
import polars as pl
import pytest
from ncaa_mbb_01_schedules_scrape import discover_season
from ncaa_mbb_04_rosters_scrape import capture_rosters
from ncaa_mbb_05_datasets_build import (
    ROSTER_COLUMNS,
    SCHEDULE_COLUMNS,
    TEAMS_COLUMNS,
...
def test_dataset_path_layout(root: Path) -> None:
    ...
    assert rel(dataset_path(root, "mbb", "schedules", "html", 2026, 1)) == "mbb/schedules/html/2026/1.html"
    assert rel(dataset_path(root, "mbb", "rosters", "json", 2026, 1)) == "mbb/rosters/json/2026/1.json"
    # ...parquet is ONE compiled file per season, one directory level up.
    assert rel(dataset_path(root, "mbb", "schedules", "parquet", 2026)) == "mbb/schedules/parquet/2026.parquet"
    assert rel(dataset_path(root, "mbb", "rosters", "parquet", 2026)) == "mbb/rosters/parquet/2026.parquet"
    # teams is one file per season in every format.
    for fmt in ("html", "json", "parquet"):
        assert rel(dataset_path(root, "mbb", "teams", fmt, 2026)) == f"mbb/teams/{fmt}/2026.{fmt}"
...
def test_season_parquet_carries_ids_and_readable_names(root: Path) -> None:
    ...
    assert df.get_column("team").unique().to_list() == ["Illinois"]
...
def test_schedule_carries_espn_ids_for_both_sides(root: Path) -> None:
    ...
    # The swept team resolves for every row (Illinois -> ESPN 356).
    assert df.get_column("espn_team_id").unique().to_list() == ["356"]
...
def test_roster_carries_the_teams_espn_id(root: Path) -> None:
    ...
    assert df.schema["espn_team_id"] == pl.Utf8
    assert df.get_column("espn_team_id").unique().to_list() == ["356"]  # Illinois
...
def test_build_teams_writes_all_three_formats_with_names_and_division(root: Path) -> None:
    ...
    assert "<table>" in html and "Illinois" in html
...
def test_build_teams_joins_a_present_espn_crosswalk(root: Path, monkeypatch) -> None:
    ...
    fake = pl.DataFrame(
        {
            ...
            "espn_display_name": ["Illinois Fighting Illini", "nope"],
            "espn_mascot": ["Fighting Illini", "nope"],
        },
    ...
    assert row["espn_display_name"] == "Illinois Fighting Illini"
    assert row["espn_mascot"] == "Fighting Illini"
...
def test_rebuild_missing_backfills_rosters_from_the_legacy_payload(root: Path) -> None:
    ...
    assert rebuilt.get_column("team").unique().to_list() == ["Illinois"]
